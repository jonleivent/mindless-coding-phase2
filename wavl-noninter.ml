...
let gofis t g =
  match t with
  | Missing -> false
  | Node (ug, _, _, _) -> if g then if ug then true else false else if ug then false else true
...
let rot1 lt x rt g =
  ...
  | Node (_, lc, d, rc) ->
    (match rc with
     ...
     | Node (rg, lc0, d0, rc0) ->
       if rg
       then Node (g, lc, d, (Node (false, (setgap rc false), x, (setgap rt false))))
...
let rot2 lt x rt g =
  ...
  | Node (_, lc, d, rc) ->
    (match lc with
     ...
     | Node (lg, lc0, d0, rc0) ->
       if lg
       then Node (g, (Node (false, (setgap lt false), x, (setgap lc false))), d, rc)
...
| Node (ug, lc, d, rc) ->
  (match delmin lc with
   ...
   | MinDeleted (min, dr) ->
     let (t0, d0) = dr in
     (match d0 with
      | DSameK -> MinDeleted (min, ((Node (ug, t0, d, rc)), DSameK))
      | DLowerK ->
        if gofis rc false
        then if gofis lc true
             then (match tryLowering rc with
                   | TLtooLow -> MinDeleted (min, (drot1 t0 d rc ug))
                   | TLlowered t' -> MinDeleted (min, ((Node (true, t0, d, t')), DLowerK)))
             else MinDeleted (min, ((Node (ug, t0, d, rc)), DSameK))
        else MinDeleted (min, ((Node (true, (setgap t0 (getgap lc)), d, (setgap rc false))), DLowerK))))
...
| Node (ug, lc, d, rc) ->
  (match delmax rc with
   ...
   | MaxDeleted (max, dr) ->
     let (t0, d0) = dr in
     (match d0 with
      | DSameK -> MaxDeleted (max, ((Node (ug, lc, d, t0)), DSameK))
      | DLowerK ->
        if gofis lc false
        then if gofis rc true
             then (match tryLowering lc with
                   | TLtooLow -> MaxDeleted (max, (drot2 lc d t0 ug))
                   | TLlowered t' -> MaxDeleted (max, ((Node (true, t', d, t0)), DLowerK)))
             else MaxDeleted (max, ((Node (ug, lc, d, t0)), DSameK))
        else MaxDeleted (max, ((Node (true, (setgap lc false), d, (setgap t0 (getgap rc)))), DLowerK))))
